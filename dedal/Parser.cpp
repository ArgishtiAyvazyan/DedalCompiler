...
ExecutableCode Parser::executavle_code() const
{
	return ExecutableCode{m_stack_size, m_data_segment_buffer, m_code_segment_buffer};
}
...
void Parser::parse_forward_declaration(const std::cmatch& tokens)
{
    ...
    m_variable_name_to_id_map[std::string{ variable_name }] = m_free_variable_id;
    m_data_segment_buffer << m_free_variable_id;
    ++m_free_variable_id;
...
Parser::code_iterator Parser::parse_code_segment(code_iterator begin, code_iterator end)
{
    ...
    for (; it != end; ++it)
    {
        ...
        const auto instruction = find_instruction_by_name(tokens[0]);
        ...
        case Instructions::cmp:
			...
			m_code_segment_buffer << util::to_int_t(tokens[1]) << util::to_int_t(tokens[2]);
        break;
            // 1 argument
        ...
        case Instructions::call:
			...
			m_code_segment_buffer << util::to_int_t(tokens[1]);
            break;
			// label
		case Instructions::label: {
			const auto& label_name = tokens[2];
			const auto search_lable = m_label_name_to_id_map.find(label_name);
			// label alredy exists.
			if (search_lable != std::end(m_label_name_to_id_map))
			{
				throw std::invalid_argument{ "Label duplicate.\n"s + label_name };
			}
...
Instructions Parser::find_instruction_by_name(const std::string& instruction_name)
{
    return s_string_to_instruction_enum_map.at(instruction_name);
}
